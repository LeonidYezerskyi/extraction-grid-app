...
def render_sidebar(canonical_model, topic_aggregates: List[Dict[str, Any]]):
    ...
    # Coverage tier
    # Check reset flag first - if set, force index 0 (empty)
    if st.session_state.get('_reset_filters_flag', False):
        coverage_tier_index = 0
        # Clear the reset flag after using it
        st.session_state['_reset_filters_flag'] = False
    elif 'coverage_tier_filter' not in st.session_state:
        coverage_tier_index = 0
    else:
    ...
    if st.session_state.get('_reset_filters_flag', False):
        tone_rollup_index = 0
        # Don't clear the flag here - it will be cleared after coverage_tier
    elif 'tone_rollup_filter' not in st.session_state:
        tone_rollup_index = 0
    else:
    ...
    if st.sidebar.button("🔄 Reset Filters", use_container_width=True, key='reset_filters_button', help="Clear all filters and search query"):
        ...
        st.session_state[SESSION_KEYS['search_query']] = ''
        st.session_state['participant_filter_patterns'] = []
        # Set reset flag to force widgets to use empty values
        st.session_state['_reset_filters_flag'] = True
    ...
    if canonical_model and topic_aggregates:
        ...
        filtered_aggregates_for_export = filter_topics(
            topic_aggregates,
            st.session_state.get(SESSION_KEYS['filters'], {}),
            st.session_state.get(SESSION_KEYS['search_query'], ''),
            canonical_model
        )
        # Get selected aggregates from filtered results
        selected_aggregates = [t for t in filtered_aggregates_for_export if t['topic_id'] in selected_topic_ids]
        ...
        # Empty digest if no canonical_model or topic_aggregates
        digest_artifact = {
            'takeaways': [],
            'topic_cards': [],
            'metadata': {
